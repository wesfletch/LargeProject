...
userRouter.post('/register',(req,res)=>{
    ...
    User.findOne({email},(err,user)=>{
        ...
        if(!Validator.equals(password,password2)){
            return res.status(403).json({message : {msgBody : "Error: Passwords must match.", msgError: "N/A"}});
        }
        if (!email || !password || !name) {
            return res.status(405).json({message : {msgBody : "Error: Please provide an email, name, and password.", msgError: "N/A"}});
        }
...
userRouter.post('/add/', passport.authenticate('jwt',{session : false}),async(req,res)=>{
    User.findOne({email: req.body.email}, function(err,doc) { 
        ...
        if(!doc){ //Checks if email is in database
            return res.status(404).json({message : {msgBody : "Error: User not found.", msgError: "N/A"}});
        }
        if(doc){ //Checks if friend already exists in friend's list
            if(req.user.friends.indexOf(doc.id.toString()) === -1) {
                ...
                return res.status(401).json({message : {msgBody : "Error: User is already your friend.", msgError: "N/A"}});
            }
        }
    });
});
